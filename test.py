...
def clean_ai_response(text):
    """【終極 AI 幻覺清洗器】徹底抹殺 JSON 殘骸與英文草稿"""
    if not isinstance(text, str): return str(text)
    raw = text.strip()
    
    # 1. 移除推理模型特有的 JSON 外殼
    raw = re.sub(r'\{?"role"\s*:\s*"assistant"\s*,?\s*"reasoning_content"\s*:\s*".*?"\s*,\s*"content"\s*:\s*"', '', raw, flags=re.DOTALL)
    raw = re.sub(r'","tool_calls"\s*:\s*\[\]\}?', '', raw)
    
    # 2. 移除 <think> 標籤
    raw = re.sub(r"<think>.*?</think>", "", raw, flags=re.DOTALL | re.IGNORECASE)
    
    # 3. 物理截斷：尋找第一句正確的標題 (【)，拋棄前面的所有英文廢話
    if "【" in raw:
        raw = raw[raw.find("【"):]
        
    # 4. 移除逐行的純英文思考 (如 "Thus our answer:", "Not in JSON")
    cleaned_lines = []
    for line in raw.split('\n'):
        ...
        # 只有英文字母，沒有中文字母，且不是 Ticker 列表 -> 當作英文草稿刪除！
...
def extract_stock_sentiment_output(text):
    ...
    if not body: body = "市場消息面未有一面倒優勢，利好與風險並存，建議等待更多催化消息再作判斷。"
    return found_label, body

# ==========================================
# 3. 另類數據資料源 (精準真實版 + 6大維度)
# ==========================================
@st.cache_data(ttl=1800, show_spinner=False)
def fetch_reddit_sentiment():
    """【第 1 Part】Reddit 討論度 + Trend (24h) | Top 10"""
    try:
        ...
        if res.status_code == 200:
            ...
            if results:
                ...
                return pd.DataFrame(rows), '🟢 ApeWisdom API (含 Trend 數據)'
    except: pass
    return pd.DataFrame(), '🔴 網絡異常 (為保證準確，已拒絕顯示假數據)'
...
def fetch_insider_buying():
    """【第 4 Part】高層真金白銀買入 (Top 10 Ticker + Date + Source，完全準確版)"""
    try:
...
def fetch_congress_trades():
    """【第 5 Part】國會議員交易 (Top 10 Ticker + Date + Source，完全準確版)"""
    trades = []
    # 參議院 (Senate)
    try:
...
def analyze_news_ai(news_list):
    ...
    你係香港頂級金融分析師。
    【絕對強制指令 - 否則系統會崩潰】：
    1. 報告必須 100% 使用香港廣東話（例如：嘅、啲、咁、睇好、大戶、散水）。
    2. 絕對不允許輸出任何英文思考過程（如 'Not in JSON', 'Thus', 'Positive', 'Negative'）。
    3. 絕對不允許輸出 JSON 格式或 markdown code block。
    4. 必須直接輸出最終報告，由「【📉 近月市場焦點總結】」開始。
    """.strip()
    
    user_prompt = f"請根據以下財經新聞寫一份地道嘅香港廣東話分析：\n{news_text}"
...
def analyze_alt_data_ai(reddit_df, jp_df, x_df, insider_df, congress_df, twits_df):
    ...
    【絕對強制指令 - 否則系統會崩潰】：
    1. 報告必須 100% 使用香港廣東話。
    2. 絕對不允許輸出任何英文思考過程或 JSON 殘骸。
    3. 包含以下詞語：瘋狂吸籌、春江鴨、人踩人風險、接火棒。
...
def analyze_single_stock_sentiment(ticker, news_items):
    ...
    你係香港 AI 股評人。
    【鐵血指令】：
    1. 絕對不允許輸出 JSON 代碼及思考草稿。
    2. 第一行必須且僅能是這五個標籤之一：【🔥 極度看好】 或 【📈 偏向樂觀】 或 【⚖️ 中性觀望】 或 【📉 偏向悲觀】 或 【🧊 極度看淡】。
...
if app_mode == '🎯 RS x MACD 動能狙擊手':
        ...
        if not raw_data.empty:
            ...
            if enable_rs or enable_macd or enable_sma:
                ...
                if len(final_df) > 0:
                    ...
                    cols = ['Ticker'] + [c for c in ['RS_階段', 'MACD_階段', 'Company', 'Sector', 'Market Cap'] if c in final_df.columns]
    ...
    with c4:
        st.markdown('**4. 高層 Insider 買入 (Top 10 Ticker + Date + Source)**')
    ...
    c5, c6 = st.columns(2)
    with c5:
        st.markdown('**5. 國會議員交易 (Top 10 Ticker + Date + Source)**')
