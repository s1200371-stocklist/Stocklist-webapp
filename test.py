...
def analyze_alt_data_ai(reddit_df, twits_df, insider_df, congress_df):
    ...
    【絕對強制規範】：
    1. 必須用地道「香港廣東話口語（Cantonese）」寫報告，語氣要極度專業、深入，同時保留生動嘅形容詞（例如：瘋狂吸籌、大戶散戶大隻揪、人踩人風險、春江鴨、佩洛西概念股）。
    2. 絕對禁止輸出任何 JSON、字典、或英文思考過程。
    3. 必須嚴格按照用戶提供嘅 Markdown 格式輸出，段落要分明，好似一篇頂級財經長文專欄咁。
    4. 必須喺分析入面精準引用我提供嘅真實數據（例如具體提及次數、買入金額、CEO或政客嘅人名）。
    5. 【核心指令】：不設字數上限！請盡情發揮，每段分析都要極度詳細同深入探討背後嘅資金博弈邏輯，越長篇越好！"""
    ...
    請嚴格根據以下格式輸出深度報告（不設字數上限，請直接填寫內容，每一部分請寫出極度豐富嘅長篇分析）：
    
    【🕵️ 另類數據 AI 偵測深度報告】

    1. 【🔥 散戶雙引擎：流動性正喺度衝擊邊個板塊？】
    （綜合 Reddit 同 StockTwits 嘅名單。詳細列出最受關注嘅 2-3 隻 Meme 股，並引用具體討論熱度數據。深入長篇分析散戶情緒係偏向極度貪婪定係避險，佢哋炒緊啲咩具體概念或板塊輪動？）

    2. 【🏛️ 聰明錢與政客追蹤：終極內幕買緊乜？】
    （詳細分析高層 Insider 同國會議員嘅買入名單。必須點名邊位政客或者 CEO 買咗咩股票，同埋涉資幾大。深入長篇解讀呢班「春江鴨」背後嘅資金佈局邏輯，佢哋係咪趁大市震盪靜靜雞撈底？）

    3. 【🎯 終極四維共振：最強爆發潛力股與高危陷阱】
    （對比散戶同大戶名單，挑選「散戶炒緊 + 大戶/政客偷偷入緊」嘅黃金交叉股票，長篇解釋爆發潛力。同時，必須點名 1 隻「只有散戶狂炒，但大戶完全冇入局甚至可能走緊」嘅高危股票，嚴重警告人踩人風險。）
    """
    try:
        # Timeout 加長至 80 秒，等 AI 有充分時間嘔出長篇大論
        response = requests.post('https://text.pollinations.ai/', json={'messages': [{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': user_prompt}], 'model': 'openai'}, timeout=80)
...
# --- 模組 C 顯示 ---
    ...
    if st.button('🚀 啟動 AI 四維交叉博弈分析', type='primary', use_container_width=True):
        with st.spinner('🧠 AI 正在進行散戶 vs 政客大戶 4 維度深度分析... (不設字數上限，生成可能需要 30-60 秒)'):
