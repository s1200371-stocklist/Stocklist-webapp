...
def calculate_all_indicators(tickers, sma_short, sma_long, close_condition, batch_size=200, _progress_bar=None, _status_text=None):
    ...
    for i in range(0, total_tickers, batch_size):
        ...
        if _status_text:
            _status_text.markdown(f'**階段 2/3**: 計緊技術指標... (`{min(i+batch_size, total_tickers)}` / `{total_tickers}`)')
        if _progress_bar:
            _progress_bar.progress(min(1.0, (i + batch_size) / max(total_tickers, 1)))
        try:
            ...
            for ticker in batch_tickers:
                ...
                if ticker in close_prices.columns and not close_prices[ticker].dropna().empty:
                    ...
                    if len(stock_price) > max(sma_short, sma_long) + 1:
                        ...
                        if float(rs_line.iloc[-1]) > float(rs_ma_25.iloc[-1]):
                            rs_stage = '🚀 啱啱突破' if float(rs_line.iloc[-2]) <= float(rs_ma_25.iloc[-2]) else '🔥 已經突破'
                        elif float(rs_line.iloc[-1]) >= float(rs_ma_25.iloc[-1]) * 0.95:
                            rs_stage = '🎯 就快突破 (<5%)'
                        ...
                        if float(macd_line.iloc[-1]) > float(signal_line.iloc[-1]):
                            macd_stage = '🚀 啱啱突破' if float(macd_line.iloc[-2]) <= float(signal_line.iloc[-2]) else '🔥 已經突破'
                        elif abs(float(macd_line.iloc[-1]) - float(signal_line.iloc[-1])) <= max(abs(float(signal_line.iloc[-1])) * 0.05, 1e-9):
                            macd_stage = '🎯 就快突破 (<5%)'
...
def fetch_fundamentals(tickers, _progress_bar=None, _status_text=None):
                ...
                def fmt_val(vals, is_sales=False):
                    ...
                    for v in vals:
                        if v is None:
                            ...
                            out.append(f'{v/1e9:.2f}B' if v >= 1e9 else (f'{v/1e6:.2f}M' if v >= 1e6 else f'{v:.0f}'))
                        else:
                            out.append(f'{v:.2f}')
                    return ' | '.join(out)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            completed += 1
            if _status_text:
                _status_text.markdown(f'**階段 3/3**: 攞緊最新財報數據... (`{completed}` / `{total_tickers}`)')
            if _progress_bar:
                _progress_bar.progress(min(1.0, completed / max(total_tickers, 1)))
            ...
            if hasattr(tkr, 'news') and isinstance(tkr.news, list):
                for item in tkr.news[:5]:
                    ...
                    if title and title not in seen:
                        ...
                        news_items.append({'來源': publisher, '新聞標題': title, '內文摘要': str(summary)[:200]})
    except Exception:
        pass
    return news_items
...
def analyze_news_ai(news_list):
    ...
    system_prompt = '你係香港中環頂級金融分析師。必須用香港廣東話口語寫報告。絕對禁止輸出JSON或編程代碼。直接輸出Markdown報告，開頭第一句必須係：【📉 近月市場焦點總結】。'
    user_prompt = f'分析以下新聞：\n{news_text}\n1. 【📉 近月市場焦點總結】：150-200字總結大市走勢同情緒。\n2. 【🚀 潛力爆發股全面掃描】：搵出所有有潛力或炒作嘅Ticker，每隻用1-2句廣東話解釋點解睇好。'
    try:
...
def analyze_alt_data_ai(reddit_df, twits_df, insider_df, congress_df):
    ...
    【絕對強制規範】：
    1. 必須用地道「香港廣東話口語（Cantonese）」寫報告，語氣要極度專業、深入，同時保留生動嘅形容詞（例如：瘋狂吸籌、大戶散戶大隻揪、人踩人風險、佩洛西概念股）。
    2. 絕對禁止輸出任何 JSON、字典、或英文思考過程。
    3. 必須嚴格按照用戶提供嘅格式輸出，段落要分明，要好似一篇頂級財經專欄咁。
    4. 必須喺分析入面引用真實數據（例如具體提及次數、買入金額、人名）。"""
    
    user_prompt = f"""
    請極度深入咁交叉分析以下四大另類數據陣營：
    [散戶數據 1：Reddit WSB 熱門 (過去24小時)]:\n{r_str}\n
    [散戶數據 2：StockTwits 熱搜 (即時)]:\n{t_str}\n
    [大戶數據 1：高層 Insider 買入 (過去30日)]:\n{i_str}\n
    [大戶數據 2：國會議員交易 (過去45日)]:\n{c_str}\n
    
    請嚴格根據以下格式輸出深度報告（直接填寫內容，字數要充足，每段最少 200-300 字）：
    
    【🕵️ 另類數據 AI 偵測深度報告】

    1. 【🔥 散戶雙引擎：流動性正喺度衝擊邊個板塊？】
    （綜合 Reddit 同 StockTwits 嘅名單。詳細列出最受關注嘅 2-3 隻 Meme 股，並引用具體討論熱度數據。深入分析散戶情緒係偏向極度貪婪定係避險，佢哋炒緊啲咩具體概念或板塊輪動？）

    2. 【🏛️ 聰明錢與政客追蹤：終極內幕買緊乜？】
    （詳細分析高層 Insider 同國會議員嘅買入名單。必須點名邊位政客或者 CEO 買咗咩股票，同埋涉資幾大。深入解讀呢班「春江鴨」背後嘅資金佈局邏輯，佢哋係咪趁大市震盪靜靜雞撈底？）

    3. 【🎯 終極四維共振：最強爆發潛力股與高危陷阱】
    （對比散戶同大戶名單，挑選 1-2 隻「散戶炒緊 + 大戶/政客偷偷入緊」嘅黃金交叉股票，詳細解釋爆發潛力。同時，必須點名 1 隻「只有散戶狂炒，但大戶完全冇入局甚至可能走緊」嘅高危股票，警告人踩人風險。）
    """
    try:
        response = requests.post('https://text.pollinations.ai/', json={'messages': [{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': user_prompt}], 'model': 'openai'}, timeout=60)
        return clean_ai_response(response.text) if response.status_code == 200 else '⚠️ AI 分析暫時離線或伺服器繁重，請稍後再試。'
    except Exception as e:
...
# --- 模組 B 顯示 ---
    ...
    st.markdown('系統自動爬取近一個月嘅財經熱門新聞，交俾 AI 用廣東話分析。')
